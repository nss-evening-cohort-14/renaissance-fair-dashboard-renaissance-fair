const showEvents = (array) => {
  ...
  // array.forEach((item) => {
  //   document.querySelector('#content-container').innerHTML += `
  //   <div class="item card border-0 bg-transparent">
  ...
  array.forEach((item) => {
    document.querySelector('#content-container').innerHTML += `<div class="item card border-0 bg-transparent" style="width: 18rem;">
    <img src="${item.image}" class="card-img-top top" alt="event image">
    <div class="card-body middle">
      <h5 class="card-title" id="event-title--${item.firebaseKey}">${item.title}</h5>
